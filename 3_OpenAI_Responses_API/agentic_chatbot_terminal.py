...
# Import the necessary modules for making HTTP requests and handling errors.
...
# Look up a location with Open-Meteo's free Geocoding API and return its IANA timezone.
# This API does not require an account or API key.
def get_location_timezone(location):
    ...
    try:
        ...
        return {"error": "The timezone service is currently unavailable.", "details": str(error)}
...
tools = [
    # Define a hosted web-search tool for current travel information.
    {"type": "web_search"},
    # Define a function tool called get_location_timezone.
    {
    "type": "function",
    "name": "get_location_timezone",
    "description": ("Get the IANA timezone for a city, region, or travel destination using the free Open-Meteo Geocoding API."),
    "parameters": {
        "type": "object",
        "properties": {
            "location": {
                "type": "string",
                "description": ("The city, region, or destination whose timezone is needed, for example 'Toronto' or 'Tokyo, Japan'.")
                }
        },
        "required": ["location"],
...
while True:
    ...
    response_first = client.responses.create(
        model="gpt-5.4-mini",
    ...
    if function_outputs:
        ...
        response_second = client.responses.create(
            model="gpt-5.4-mini",
            reasoning={"effort": "low"},
            max_output_tokens=400,
            instructions=instructions,
